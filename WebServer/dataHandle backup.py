...
class data(object):
	...
	def __from_edf(self):
		...
		del f
		del self.file
		raw = mne.io.read_raw("tmp.edf")
		print(type(raw))
		#questo è un mne object quindi ovviamente qua non va
		data = raw.load_data()
		...
		for i in range(len(names)):
			if names[i] in eeg_name:
				print("eeg")
				eeg_names = eeg_names  + names[i] + " "
				tmp=sig[i].tolist()
			...
			elif names[i] in eog_name:
				print("eog")
				eog_names = eog_names  + names[i] + " "
				tmp = sig[i].tolist()
				eog = eog+str(tmp) +"\n"


			elif names[i] in ecg_name:
				print("ecg")
				ecg_names = ecg_names  + names[i] + " "
				tmp = sig[i].tolist()
				ecg = ecg+str(tmp) +"\n"


			elif names[i] in emg_name:
				print("emg")
				emg_names = emg_names + names[i] + " "
				tmp = sig[i].tolist()
				emg = emg + str(tmp) + "\n"

			elif names[i] in imp_name:
				print("imp")
				imp_names = imp_names  + names[i] + " "
				tmp = sig[i].tolist()
				imp = imp+str(tmp) +"\n"
		del names
		del sig
